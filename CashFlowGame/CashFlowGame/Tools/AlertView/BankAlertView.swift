...
let ALERT_HEIGHT = CGFloat(260.0)
let LABEL_HEIGHT = CGFloat(30.0)

class BankAlertView: UIView,UITextFieldDelegate {
...
//            pwdTextField?.addTarget(self, action: #selector(pwdTextFieldHandle), for: .valueChanged)
    ...
    @objc func sureBtnHandle() {
        ...
        if self.totalString?.count == 0 || !isRight {
            
            let alert = UIAlertView.init(title: "提示", message: "未办理业务", delegate: self, cancelButtonTitle: "OK")
            alert.show()
